    ...
    public class Robot
    {
       ...
       // private const string CONTROL_NOT_FOUND_EXCEPTION = "The specific control is not found!!";
        ...
        public void AssertText(string name, string text)
        {
            ...
            Assert.AreEqual(text.Trim(), element.Text.Trim());
        }
        ...
        public void AssertDataGridViewRowDataBy(string name, int rowIndex, string[] data)
        {
            ...
            {
                string dataFromDataGridView = rowDatas[i].Text.Replace("(null)", "").Replace("\n", "").Replace("\r", "").ToString();
                Assert.AreEqual(data[i - 1].Trim(), dataFromDataGridView.Trim());
            }
        }
        ...
        public void AssertDataGridViewRowCountBy(string name, int rowCount)
        {
            ...
            if (element != null)
            {
                if (element.GetCurrentPattern(GridPattern.Pattern) is GridPattern gridPattern)
                {
        ...
        // test
        public void AssertListBoxCountBy(string name, int rowCount)
        {
            var dataGridView = _driver.FindElementByAccessibilityId(name);
            Point point = new Point(dataGridView.Location.X, dataGridView.Location.Y);
            AutomationElement element = AutomationElement.FromPoint(point);

            while (element != null && element.Current.LocalizedControlType.Contains("list") == false)
            {
                element = TreeWalker.RawViewWalker.GetParent(element);
            }
        ...
        //test
        public void AssertTabPageCountBy(string name, int rowCount)
        {
            var dataGridView = _driver.FindElementByAccessibilityId(name);
            Point point = new Point(dataGridView.Location.X, dataGridView.Location.Y);
            AutomationElement element = AutomationElement.FromPoint(point);

            while (element != null && element.Current.LocalizedControlType.Contains("tabcontrol") == false)
            {
                element = TreeWalker.RawViewWalker.GetParent(element);
            }
